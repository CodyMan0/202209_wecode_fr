...
<details>
  <summary>1. 폼을 만드는 에디터를 어떻게 만들지?</summary>
<br>

- **상황 설명** : 관리자가 설문조사를 만드는 에디터 페이지, 만든 설문지의 링크를 고객에게 보내 고객의 설문을 받아야하는 고객 페이지가 필요.

- **정의** : 프론트 단에서 JSON 객체 형식으로 데이터를 가공할 수 있으면 된다.
- **해결 과정** :

  1. **survey.js**를 사용하여 손쉽게 JSON 형식으로 프론트단에서 가공하여 만든 데이터를 활용하여 UI로 보여주고려 했으나 JSON 데이터를 UI로 변환하는 것은 무료이지만 사용자가 직접 UI를 통해 JSON 데이터를 만드는 부분은 유료 서비스여서 **보류**

  2. **react-hook-form**을 활용하여 JSON 형식의 데이터로 만들어보자.

- **공부한 부분** : survey.js , useForm
- **경험** :
</details>

<details>
<summary>2. 폼 안에 있는 인풋 상태관리 및 중첩 인풋 관리 그리고 개별 관리</summary>
<br>

- **상황 설명** :
- **정의** : 상태에 따라 렌더링이 일어나는 useState 훅이 아닌 useRef를 사용하면 되지 않을까.
- **해결 과정** :

1. 모든 form들의 state를 관리할 수가 없다. 어떻게 해야하는 걸까? (useForm)
2. 인풋이 공통 컴포넌트여서 고유한 이름을 어떻게 줘야하지? (useForm > register)
...
<details>
<summary>3. 왼편에 어떤 선택 질문을 누르는지에 따라 그에 따른 UI가 그려지도록</summary>
<br>
...
<details>
<summary>4. 설문지를 만들고 제출을 눌렀는데 데이터가 서버로 2번 이상 전송되는 문제.</summary>
<br>
...
<details>
<summary>6. 관리자 설정하러 가기를 눌렀을때 해당 페이지 인풋 유효성 검사</summary>
<br>
